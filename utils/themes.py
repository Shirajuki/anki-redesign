...
def get_theme(theme: str) -> dict:
    ...
    if not theme_colors.get("BUTTON_FOCUS_BG", False):
        theme_colors["BUTTON_FOCUS_BG"] = [
            "Button Focus Background", "#0093d0", "#0093d0", "--button-focus-bg"]
    if not theme_colors.get("FOCUS_SHADOW", False):
        theme_colors["FOCUS_SHADOW"] = ["Focus Shadow",
                                        "#ff93d0", "#0093d0", "--focus-shadow-color"]
    if theme_colors.get("BUTTON_BG", False):
        theme_colors["BUTTON_BG"] = theme_colors["BUTTON_BG"][:-1] + \
            ["--button-bg"]

    # Fix legacy themes
    # TODO: Add more fixes
    if not theme_colors.get("FG", False):
        theme_colors["FG"] = theme_colors["TEXT_FG"][:-1] + ["--fg"]
    if not theme_colors.get("CANVAS", False):
        theme_colors["CANVAS"] = theme_colors["WINDOW_BG"][:-1] + ["--canvas"]
    if not theme_colors.get("CANVAS_CODE", False):
        theme_colors["CANVAS_CODE"] = theme_colors["FRAME_BG"][:-
                                                               1] + ["--canvas-code"]
    if not theme_colors.get("CANVAS_ELEVATED", False):
        theme_colors["CANVAS_ELEVATED"] = theme_colors["FRAME_BG"][:-1] + \
            ["--canvas-elevated"]
    if not theme_colors.get("FG_SUBTLE", False):
        theme_colors["FG_SUBTLE"] = theme_colors["DISABLED"][:-
                                                             1] + ["--fg-subtle"]
    if not theme_colors.get("FG_DISABLED", False):
        theme_colors["FG_DISABLED"] = theme_colors["DISABLED"][:-
                                                               1] + ["--fg-disabled"]
    if not theme_colors.get("FG_LINK", False):
        theme_colors["FG_LINK"] = theme_colors["LINK"][:-1] + ["--fg-link"]
